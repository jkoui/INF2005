...
def load_wav_from_file(file_or_path):
    """
    Accepts either a Flask FileStorage (with .stream) or a plain path string.
    Returns:
      flat_bytes (np.uint8 1-D),
      params (dict with nchannels, sampwidth, framerate, nframes, comptype, compname),
      raw_frames (bytes)
    """
    # Decide whether input is FileStorage or file path
    if hasattr(file_or_path, "stream"):
        wf = wave.open(file_or_path.stream, "rb")
    else:
        wf = wave.open(file_or_path, "rb")  # assume it's a path string
...
# -------- MP4 (frame-level) --------
def load_video_from_file(file_storage):
    """
    Reads video frames into flat bytes.
    Returns:
      flat (np.uint8 1-D),
      meta (dict: width, height, frames, fps, shape),
